...
def get_data_from_mongo():
    """
    Busca os dados do MongoDB. Esta é a versão final e robusta que lida
    com dados de fuso horário misturados.
    """
    try:
        ...
        # Versão robusta para tratar datas:
        # 1. Converte a coluna para datetime
        s_emissao = pd.to_datetime(df['emissao'])
        # 2. Padroniza tudo para UTC e depois converte para o fuso local.
        #    Isso resolve o erro de 'naive' vs 'aware' timestamps.
...
def main():
    ...
    df_ano_atual = df[df['emissao'] >= hoje.replace(month=1, day=1, hour=0, minute=0, second=0, microsecond=0)]
    ...
    pdf.create_kpi_box('Faturamento Mês Atual', df_mes_atual['valor_total_nota'].sum(), x=10, y=45)
    pdf.create_kpi_box('Faturamento Mês Anterior', df_mes_anterior['valor_total_nota'].sum(), x=78, y=45)
    pdf.create_kpi_box('Acumulado Ano', df_ano_atual['valor_total_nota'].sum(), x=146, y=45)
    pdf.ln(30)
    
    print("Gerando gráficos da página 1...")
    create_bar_chart(df_mes_atual.groupby('filial_nome')['valor_total_nota'].sum(), 'Faturamento por Filial (Mês Atual)', 'chart_p1_1.png')
    pdf.image('chart_p1_1.png', x=10, w=190)
    create_bar_chart(df_mes_anterior.groupby('filial_nome')['valor_total_nota'].sum(), 'Faturamento por Filial (Mês Anterior)', 'chart_p1_2.png')
    pdf.image('chart_p1_2.png', x=10, w=190)
    create_bar_chart(df_ano_atual.groupby('filial_nome')['valor_total_nota'].sum(), 'Faturamento por Filial (Acumulado Ano)', 'chart_p1_3.png')
    ...
    for filial in FILIAIS_ORDEM:
        ...
        if df_filial.empty:
            ...
            pdf.cell(0, 10, 'Nenhuma nota fiscal encontrada.', 0, 1)
        else:
            headers = ['Emissão', 'Nota Fiscal', 'Cliente', 'Valor Total']
            column_widths = [25, 25, 90, 30]
            table_data = [[row['emissao'].strftime('%d/%m/%Y'), row['numero_nota'], str(row['EMPRESA'])[:50], f"R$ {row['valor_total_nota']:,.2f}"] for _, row in df_filial.iterrows()]
