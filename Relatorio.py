...
# --- FUNÇÕES DE BANCO DE DADOS E GRÁFICOS (sem alterações) ---
...
def create_bar_chart(data, title, filename):
    if data.empty or data.sum() == 0:
        ...
        # A cor é convertida para o formato 0.0-1.0 dividindo por 255
        ax.set_title(title, loc='left', color=[c/255 for c in COR_SECUNDARIA], fontsize=10) # <-- CORREÇÃO AQUI
...
def create_line_chart(data, title, filename):
    ...
    if data.empty:
        ...
        # Título do gráfico com a cor convertida para o formato 0.0-1.0
...
# --- CLASSE PDF COM ESTILO APRIMORADO ---

class PDF(FPDF):
    def header(self):
        # --- ALTERAÇÃO DE ESTILO: Placeholder para logo e título com cor ---
    ...
    def create_kpi_box(self, title, value, x, y):
        # --- ALTERAÇÃO DE ESTILO: Cores e fontes nos KPIs ---
        self.set_xy(x, y)
    ...
    def create_styled_table(self, headers, data, column_widths):
        # --- NOVO MÉTODO: Cria tabelas com cabeçalho colorido e linhas alternadas ---
...
# --- FUNÇÃO PRINCIPAL COM LÓGICA DE GERAÇÃO DO PDF ---
...
def main():
    ...
    for filial in FILIAIS_ORDEM:
        ...
        if df_filial.empty:
            ...
            # --- ALTERAÇÃO DE ESTILO: Usando a nova tabela estilizada ---
